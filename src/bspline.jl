...
function axlprint(io::IO, f::BSplineCurve, idt::Int64 = 0)
    ...
    println(io, ">")
    indent(io, idt+2); print(io, "<dimension>",size(f.points,1),"</dimension>\n")
    indent(io, idt+2); print(io, "<number>", size(f.points,2), "</number>\n")
    indent(io, idt+2); print(io, "<order>", f.basis.order, "</order>\n")
    indent(io, idt+2); print(io, "<knots>"); for t in f.basis.knots print(io," ",t) end; print(io,"</knots>\n")
    indent(io, idt+2); print(io, "<points>\n")
    for i in 1:size(f.points,2)
        print(io, f.points[:,i], idt+2)
        print(io,"\n")
    end
...
function axlprint(io::IO, f::BSplineSurface, idt::Int64 = 0)
    ...
    println(io, ">")
    indent(io, idt+2); print(io, "<dimension>",size(f.points,1),"</dimension>\n")
    indent(io, idt+2); print(io, "<number>", size(f.points,2)," ", size(f.points,3), "</number>\n")
    indent(io, idt+2); print(io, "<order>", f.basis1.order," ", f.basis2.order, "</order>\n")
    indent(io, idt+2); print(io, "<knots>"); for t in f.basis1.knots print(io," ",t) end; print(io,"</knots>\n")
    indent(io, idt+2); print(io, "<knots>"); for t in f.basis2.knots print(io," ",t) end; print(io,"</knots>\n")    
    indent(io, idt+2); print(io, "<points>\n")


    for j in 1:size(f.points,3)
        for i in 1:size(f.points,2)
            print(io, f.points[:,i,j], idt+2)
            print(io,"\n")
        end
    end
    indent(io, idt); print(io, "</points>\n")
...
function axlprint(io::IO, f::BSplineVolume, idt::Int64 = 0)
    ...
    indent(io, idt+2); print(io, "<dimension>",size(f.points,1),"</dimension>\n")
    indent(io, idt+2); print(io, "<number>", size(f.points,2)," ", size(f.points,3), "</number>\n")
    indent(io, idt+2); print(io, "<order>", f.basis1.order," ", f.basis2.order, " ",f.basi3.order, "</order>\n")
    indent(io, idt+2); print(io, "<knots>"); for t in f.basis1.knots print(io," ",t) end; print(io,"</knots>\n")
    indent(io, idt+2); print(io, "<knots>"); for t in f.basis2.knots print(io," ",t) end; print(io,"</knots>\n")
     indent(io, idt+2); print(io, "<knots>"); for t in f.basis3.knots print(io," ",t) end; print(io,"</knots>\n") 
    indent(io, idt+2); print(io, "<points>\n")
    for  k in 1:szie(4,f.points), j in 1:size(f.points,3), i in 1:size(f.points,2)
        print(io, f.points[:,i,j,k], idt+2)
        print(io,"\n")
    end
    indent(io, idt); print(io, "</points>\n")
